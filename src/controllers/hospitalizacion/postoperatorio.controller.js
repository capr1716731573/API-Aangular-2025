...
const getAllMedicosPostoperatorio = async (req, res) => {
    let id_post = req.params.id_post;
    consulta =`select * from protocolo_operatorio_medicos pom inner join usuarios u inner join persona p 
                on u.fk_persona = p.pk_persona 
                on u.pk_usuario = pom.medico_usu_id_fk
                where pom.fk_protope=${id_post} order by pom.pk_protope_med ASC`;
    await funcionesSQL.getRows(consulta, req, res);
}
 ...
 */
const templatePaths = {
    page1: path.join(__dirname, '../../reportes/Postoperatorio/008_1.html'),
    page2: path.join(__dirname, '../../reportes/Postoperatorio/008_2.html')
};
...
const reporte008_descarga = async (req, res) => {
    /*Opcion es texto 1(Busqueda por id) 2(Busqueda por cedula o numero de identificacion)*/
    let id = req.params.id;
    const consulta = `select * from Postoperatorio_getpdf_008(1,'${id}')`;
