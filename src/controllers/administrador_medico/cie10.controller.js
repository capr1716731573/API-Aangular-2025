...
const getCie10= async (req, res) => {
    ...
    let consulta = '';
    desde = Number(desde);
    consulta =`select c.*, c_padre.desc_cie as cie_padre_nombre, c_padre.codigo_cie as cie_padre_codigo  from cie c left join cie c_padre on c.padre_cie = c_padre.pk_cie`;
...
const getCie10Bsq = async (req, res) => {
    ...
    const consulta = `select c.*, c_padre.desc_cie as cie_padre_nombre, c_padre.codigo_cie as cie_padre_codigo  from cie c left join cie c_padre on c.padre_cie = c_padre.pk_cie WHERE c.desc_cie LIKE UPPER('%${busqueda}%')`;
    await funcionesSQL.getRows(consulta, req, res);
}

const getCie10ID = async (req, res) => {
    let id = req.params.id;
    const consulta = `select c.*, c_padre.desc_cie as cie_padre_nombre, c_padre.codigo_cie as cie_padre_codigo  from cie c left join cie c_padre on c.padre_cie = c_padre.pk_cie WHERE c.pk_cie=${id}`;
